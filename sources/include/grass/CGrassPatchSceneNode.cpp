...
CGrassPatchSceneNode::CGrassPatchSceneNode(
		...
		video::IImage* grassMap, scene::IWindGenerator *windgen) : 
		 ...
		 ISceneNode((ITerrainSceneNode*)terrain, mgr, -1),
	   	 DrawDistanceSQ(GRASS_PATCH_SIZE*2.0*GRASS_PATCH_SIZE*2.0),//was 1.5
	   	 MaxDensity(6000),//was 800
	   	 Terrain(terrain),
	   	 ...
	   	 redrawnextloop(true)
{
	...
	// set position
	setPosition(core::vector3df((f32)(gridpos.X*GRASS_PATCH_SIZE),-1 , (f32)(gridpos.Z*GRASS_PATCH_SIZE)));
...
bool CGrassPatchSceneNode::Create() 
{
 ...
 // particle count 
 s32 count = 8000; // lots. we dont need them all
 ...
 for(s32 i=0; i<count; ++i)
 {
	...
	x -= GRASS_PATCH_SIZE/2.0f;//was 2.0f
	z -= GRASS_PATCH_SIZE/2.0f;//was 2.0f
	...
	core::dimension2d<f32> size((f32)(rand() % 50), 50.0f);   //height
