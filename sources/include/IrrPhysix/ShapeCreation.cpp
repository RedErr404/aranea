...
c8 textures[NUM_TEXTURES][256] = {	"../data/shaders/mask.png",
									"../data/shaders/mask.png",
									"../data/shaders/mask.png",
									"../data/shaders/mask.png",
									"../data/shaders/mask.png"  };

   int lastPlayerBulletCount, currentPlayerBulletCount = 0;
...
// Creates a sphere at the specified position, of the specified size and with the specified intial velocity (useful for throwing it)
SPhysxAndNodePair* createBullet(IPhysxManager* physxManager, scene::ISceneManager* smgr, video::IVideoDriver* driver, const core::vector3df& pos, f32 radius, f32 density, core::vector3df* initialVelocity) {
	...
	pair->SceneNode->setMaterialTexture(0, driver->getTexture(textures[rand()%NUM_TEXTURES]));
	pair->SceneNode->setMaterialFlag(video::EMF_NORMALIZE_NORMALS, true); 
	pair->PhysxObject->setLinearDamping(0);
...
// Create a sphere from the camera with an intial velocity to throw it into the scene
SPhysxAndNodePair* createBulletFromCamera(IPhysxManager* physxManager, scene::ISceneManager* smgr, video::IVideoDriver* driver, scene::ICameraSceneNode* camera, f32 radius, f32 density) {
	...
	return createBullet(physxManager, smgr, driver, t, radius, density, &vel);
