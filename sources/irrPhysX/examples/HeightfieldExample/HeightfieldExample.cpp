			...
			if (event.EventType == EET_KEY_INPUT_EVENT) {
				...
				switch (event.KeyInput.Key) {
					case KEY_KEY_W:
						...
						objects.push_back(createBox(physxManager, smgr, driver, core::vector3df(0.0f, 300.0f, -170.0f), core::vector3df(4,4,4), objectDensity));
						return true;
					case KEY_KEY_T:
						// Create a tower of 30 boxes
						createBoxTower(physxManager, smgr, driver, objects, 30, objectDensity, core::vector3df(0,300,-170));
						return true;
					case KEY_KEY_S:
						// Create a stack of boxes with 10 along the bottom
						createBoxStack(physxManager, smgr, driver, objects, 10, objectDensity, core::vector3df(0,300,-170)); 
						return true;
					case KEY_KEY_B:
						// Create a stack of boxes with 30 along the bottom
						createBoxStack(physxManager, smgr, driver, objects, 20, objectDensity, core::vector3df(0,300,-170)); 
						return true;
					case KEY_SPACE:
...
int main() {
	...
	wchar_t wstr[50];
	swprintf(wstr, 50, L"IrrPhysx(%.2f)::Heightfield Example", IRRPHYSX_VERSION);
	...
	camera->setPosition(core::vector3df(0,200,100));
	camera->setTarget(core::vector3df(0,200,-200));
	device->getCursorControl()->setVisible(false);

	// Add a light to make things look a bit nicer
	scene::ILightSceneNode* light = smgr->addLightSceneNode(0, core::vector3df(0,300,150), video::SColorf(1,1,1,1), 400);
	...
	// Create the first type of heightfield, the easy one, just add a terrain node and then create a heightfield object from the same heightmap image
	s32 smoothFactor = 0;
	// Add a terrain node
	scene::ITerrainSceneNode* terrain = smgr->addTerrainSceneNode("../media/heightmap.jpg", 0, -1, core::vector3df(-150,200,-400),	core::vector3df(0,0,0), core::vector3df(10,1,10), video::SColor(255, 255, 255, 255), 1,	scene::ETPS_17, smoothFactor);
	terrain->setMaterialTexture(0, driver->getTexture("../media/green.JPG"));
	terrain->setMaterialTexture(1, driver->getTexture("../media/detail.jpg"));
	terrain->setMaterialType(video::EMT_DETAIL_MAP);
	terrain->setMaterialFlag(video::EMF_NORMALIZE_NORMALS, true);
	// Add a physx heightfield to represent it physically
	physxManager->createHeightfieldObject(terrain, smoothFactor);

	// Create the second type of heightfield, a bit harder, but useful for when you've got terrain that's not a terrain node, maybe a mesh node (like the example below)
	// This time we have to create an array of the heightmap data to create the heightfield object (read the documentation to find out more about using this function)
	core::vector3df pos = core::vector3df(0,0,100);
	core::vector3df scale = core::vector3df(20,10,20);
	core::vector3df rot = core::vector3df(0,0,0);
	// Get our mesh
	scene::IMesh* mesh = smgr->getMesh("../media/heightmap.ms3d");
	// Add a scene node for it
	scene::ISceneNode* node = smgr->addMeshSceneNode(mesh, NULL, -1, pos, rot, scale);
	node->setMaterialFlag(video::EMF_NORMALIZE_NORMALS, true);
	node->setMaterialTexture(0, driver->getTexture("../media/green.JPG"));
	scene::IMeshBuffer* meshBuffer = mesh->getMeshBuffer(0);
	s16* data = new s16[meshBuffer->getVertexCount()];
	// The limits of an s16 are -32768 and 32768. My heightmap's vertex heights range from 0.0 and 11.15 so i can afford to multiply them by a max of around 2930 (32768/11.15) to fake the original f32 values
	f32 yMultiplier = 2930.0f;
	// Copy the mesh vertex height
	video::S3DVertex* verts = (video::S3DVertex*)meshBuffer->getVertices();
	for (u32 i = 0 ; i < meshBuffer->getVertexCount() ; i++) 
		data[i] = (s16)(verts[i].Pos.Y * yMultiplier);
	// The width and height are the number of vertices in the mesh in the X and Z directions, these i know from creating the mesh
	u32 width = 33;
	u32 height = 33;
	// Create a heightfield from the data
	physxManager->createHeightfieldObject(data, width, height, yMultiplier, pos, rot, scale, 1);	
	delete [] data;
