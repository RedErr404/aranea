...
void createExplosion(const core::vector3df& position) {
	...
	// Create an explosion
	physxManager->createExplosion(position, 200.0f, 1000.0f, 800.0f, 1.0f);

	// Create a visual representation of the explosion
	#define FRAME_TIME 100
	scene::ISceneNodeAnimator* anim = NULL;
	// create animation for explosion
	anim = smgr->createTextureAnimator(explosionTextures, FRAME_TIME, false);
	 // create explosion billboard
	scene::IBillboardSceneNode* bill = smgr->addBillboardSceneNode(smgr->getRootSceneNode(), core::dimension2d<f32>(60,60), position, -5);
	...
	anim = smgr->createDeleteAnimator(FRAME_TIME*10);
	bill->addAnimator(anim);
	anim->drop();
		...
		bool OnEvent(const SEvent& event) {
			...
			if (event.EventType == EET_KEY_INPUT_EVENT) {
				...
				switch (event.KeyInput.Key) {
					case KEY_KEY_E:
						createExplosion(core::vector3df(0,20,20));
						return true;
					case KEY_KEY_W:
...
int main() {
	...
	wchar_t wstr[50];
	swprintf(wstr, 50, L"IrrPhysx(%.2f)::Explosion Example", IRRPHYSX_VERSION);
	...
	createBox(physxManager, smgr, driver, core::vector3df(30.0f,15.0f,-30.0f), core::vector3df(30.0f,30.0f,5.0f), 0);
	...
	// Preload our explosion texture animator
	c8 tmp[64];
	for (s32 i = 1 ; i <= 10 ; ++i) {
