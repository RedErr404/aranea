...
core::array<video::ITexture*> plasmaTextures;
f32 objectDensity = 50;
core::vector3df arenaPosition(0,0,0);
core::vector3df arenaScale(100,100,100);
CGUIParamChanger* paramChanger = NULL;

void addRollingSpheres() {
	...
	// Create a number of spheres of random size with random initial velocities
	for (s32 i = 0 ; i < NUM_SPHERES ; i++) {

		// Random radius between 1, MAX_RADIUS
		f32 radius = (rand()%((s32)MAX_RADIUS-1))+1.0f;
		// Random density between 1, MAX_DENSITY
		f32 density = (rand()%((s32)MAX_DENSITY-1))+1.0f;
		// Random position within the inner bounds of the arena
		core::vector3df pos((rand()%(s32)arenaScale.X)-((arenaScale.X-MAX_RADIUS)/2.0f),
							 radius/2.0f,
							(rand()%(s32)arenaScale.Z)-((arenaScale.Z-MAX_RADIUS)/2.0f));
		// Random velocity between +/-MAX_VELOCITY
		core::vector3df velocity((rand()%((s32)MAX_VELOCITY * 2))-MAX_VELOCITY,
								  0,
								 (rand()%((s32)MAX_VELOCITY * 2))-MAX_VELOCITY);
		
		// Create the sphere
		SPhysxAndNodePair* pair = createSphere(physxManager, smgr, driver, arenaPosition + pos, radius, density, &velocity);
...
void addPlasmaBallEffect(const core::vector3df& pos) {

	#define FRAME_TIME 25
	scene::ISceneNodeAnimator* anim = NULL;
  
	// create animation for explosion
	anim = smgr->createTextureAnimator(plasmaTextures, FRAME_TIME, false);
       
	 // create explosion
	scene::IBillboardSceneNode* bill = smgr->addBillboardSceneNode(smgr->getRootSceneNode(), core::dimension2d<f32>(30,30), pos, -5);
	bill->setMaterialFlag(video::EMF_LIGHTING, false);
	bill->setMaterialTexture(0, smgr->getVideoDriver()->getTexture("../media/plasmaball/01.jpg"));
	bill->setMaterialType(video::EMT_TRANSPARENT_ADD_COLOR);
	bill->addAnimator(anim);
	anim->drop();

	// create deletion animator to automatically remove the billboard
	anim = smgr->createDeleteAnimator(FRAME_TIME*6);
	bill->addAnimator(anim);
	anim->drop();
...
int main() {
	...
	wchar_t wstr[50];
	swprintf(wstr, 50, L"IrrPhysx(%.2f)::Contact Report Example", IRRPHYSX_VERSION);
	...
	// Add an arena to contain our spheres
	createMeshBoundingBox(physxManager, smgr, driver, smgr->getMesh("../media/arena.ms3d"), arenaPosition, arenaScale, 0);
	...
	// Preload our plasmaball texture animator
	c8 tmp[64];
	for (s32 i = 1 ; i <= 6 ; ++i) {
		sprintf_s(tmp, "../media/plasmaball/%02d.jpg", i);
		plasmaTextures.push_back(smgr->getVideoDriver()->getTexture(tmp));
	}
